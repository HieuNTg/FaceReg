...
def Real_timeRecognition( usernames, model_weights, ec, cap):  
    ...
    set_background('Images\img_reg.gif')
    stframe = st.empty()  # Khung hình để hiển thị video
    stframe.image('image.jpg', width=700, channels="BGR")
    ...
    if start:
        while True:
            ...
            # Thực hiện nhận dạng khuôn mặt trên khung hình
            # sử dụng mô hình đã khởi tạo
            # và trả về danh sách các vector khuôn mặt đã nhận dạng
            filename_crop = mtcnn(frame)
            box, _ = mtcnn.detect(frame)

            # Duyệt qua danh sách vector khuôn mặt
            # và so sánh chúng với các vector khuôn mặt được lưu trữ
            if filename_crop is not None:
                box = box[0]
                result = model(filename_crop.reshape(1, 3, 160, 160))
                prob = []
            ...
            # Hiển thị khung hình với các hộp chữ nhật và tên người được nhận dạng
...
def RealTime_train( cap):
    ...
    with st.form('form1'):
        ...
        submit = st.form_submit_button('Submit')
    if username:
        st.info("Sau khi start vui lòng đợi cho quá trình train kết thúc! Trong quá trình train vui lòng quay sang trái, phải, lên và xuống.")  
        train = False
        stframe = st.empty()  # Khung hình để hiển thị video
        stframe.image('image.jpg', width=700, channels="BGR")
